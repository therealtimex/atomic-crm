 ...
 */
async function validateEmailWithAPI(
  email: string,
 ...
 */
async function validateEmailWithLibrary(
  email: string,
): Promise<EmailValidationResult> {
  try {
    ...
    try {
      ...
      if (!mxRecords || mxRecords.length === 0) {
        return {
          email,
          status: "invalid",
          reason: "No mail server (MX records not found)",
        };
      }
      ...
      return {
        email,
        status: "valid",
        reason: "Verified (syntax + MX records)",
      };
    } catch (dnsError) {
...
async function validateEmail(email: string): Promise<EmailValidationResult> {
  ...
  if (apiKey) {
    try {
      ...
      console.warn(
        `API validation failed for ${email}, falling back to library:`,
        error,
      );
 ...
 */
async function getValidationQueueSize(
  config: ValidationConfig,
): Promise<number> {
  const staleDate = new Date(
    Date.now() - config.daysStale * 86400000,
  ).toISOString();
  ...
  try {
    ...
    const { count, error } = await supabaseAdmin
      ...
      .neq("email_jsonb", "[]")
      .or(
        `email_validation_status.is.null,external_heartbeat_checked_at.lt.${staleDate}`,
      );
...
async function triggerNextBatch(
  ...
  console.log(
    `Scheduling next batch (iteration ${iteration + 1}) in ${config.delayBetweenBatches}ms...`,
  );
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Failed to trigger next batch: ${response.status} ${response.statusText}`,
      );
    } else {
      console.log(
        `Next batch triggered successfully (iteration ${iteration + 1})`,
      );
    }
  } catch (error) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (testError) {
      ...
      throw new Error(
        `Database connection failed: ${testError.message || "Unknown error"}`,
      );
    }
    ...
    const { data: _columnTest, error: columnError } = await supabaseAdmin
      .from("contacts")
      .select(
        "id, email_jsonb, email_validation_status, external_heartbeat_checked_at",
      )
      .limit(1);
    ...
    if (columnError) {
      ...
      throw new Error(
        `Column validation failed: ${columnError.message || "Unknown error"}`,
      );
    }
...
=== Validation Iteration ${iteration} ===`);
    ...
    if (iteration === 0) {
      if (queueSize === 0) {
        console.log(
          "✅ No contacts need validation, skipping session creation",
        );
        return new Response(
          JSON.stringify({
            success: true,
      ...
      const { data: newSessionId, error: sessionError } =
        await supabaseAdmin.rpc("start_validation_session", {
      ...
      if (sessionError) {
        ...
        if (
          ...
          sessionError.message?.includes("already running")
        ) {
      ...
      console.log(
        `🔄 Continuing session: ${sessionId} (iteration ${iteration})`,
      );
    }
    ...
    if (queueSize === 0) {
      ...
      if (sessionId)
        await completeSession(sessionId, "completed", "All contacts validated");
      return new Response(
        JSON.stringify({
          success: true,
          message: "All contacts validated",
    ...
    if (!contacts || contacts.length === 0) {
      ...
      if (sessionId)
        await completeSession(sessionId, "completed", "Batch empty");
      return new Response(
        JSON.stringify({
          success: true,
    ...
    console.log(
      `Processing ${contacts.length} contacts (${queueSize - contacts.length} remaining)`,
    );
    ...
    for (const contact of contacts) {
      if (
        !contact.email_jsonb ||
        !Array.isArray(contact.email_jsonb) ||
        contact.email_jsonb.length === 0
      ) {
        continue;
      }
    ...
    if (notesToCreate.length > 0) {
      ...
      if (notesError) {
        ...
        console.log(
          `Created ${notesToCreate.length} contact notes for status changes`,
        );
      }
    }
    ...
    if (remaining > 0) {
      console.log(
        `📊 Progress: ${contacts.length}/${queueSize} (${Math.round((contacts.length / queueSize) * 100)}%)`,
      );
      console.log(
        `🔄 Triggering next batch for ${remaining} remaining contacts...`,
      );
      ...
      if (supabaseUrl && serviceKey && sessionId) {
        ...
        if (sessionId)
          await completeSession(
            sessionId,
            "failed",
            "Self-trigger failed: missing config",
          );
      }
    } else {
      ...
      if (newQueueSize > 0) {
        ...
        if (supabaseUrl && serviceKey) {
          ...
          await fetch(url, {
            ...
            console.error("Failed to trigger next cycle:", err),
          );
        }
      } else {
