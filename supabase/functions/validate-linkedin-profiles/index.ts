 ...
 */
async function validateLinkedInProfile(url: string): Promise<LinkedInValidationResult> {
  try {
    // Normalize URL
    ...
    if (!url.includes('linkedin.com/in/')) {
      return { url, status: 'not_found', reason: 'Invalid LinkedIn URL format' };
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Starting LinkedIn validation (batch: ${batchSize}, stale after: ${daysStale} days)`);
    ...
    const { data: contacts, error: fetchError } = await supabaseAdmin
      ...
      .or(`linkedin_profile_status.is.null,employment_verified_at.lt.${new Date(Date.now() - daysStale * 86400000).toISOString()}`)
      .limit(batchSize);
    ...
    if (!contacts || contacts.length === 0) {
      return new Response(
        JSON.stringify({ message: 'No LinkedIn profiles need validation', validated: 0 }),
