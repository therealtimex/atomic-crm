...
export const generateCompanies = (db: Db, size = 55): Required<Company>[] => {
  return Array.from(Array(size).keys()).map((id) => {
    ...
    return {
      ...
      lifecycle_stage: hasLifecycle
        ? random.arrayElement(lifecycleStages)
        : undefined,
      company_type:
        datatype.number(100) > 30
          ? random.arrayElement(companyTypes)
          : undefined,
      qualification_status:
        datatype.number(100) > 60
          ? random.arrayElement(qualificationStatuses)
          : undefined,

      // Phase 1: External Integration (20% populated)
      external_id:
        datatype.number(100) > 80 ? random.alphaNumeric(12) : undefined,
      external_system:
        datatype.number(100) > 80
          ? random.arrayElement(["salesforce", "hubspot", "clearbit"])
          : undefined,

      // Phase 1: Contact Information (70% have email)
      email:
        datatype.number(100) > 30 ? `info@${internet.domainName()}` : undefined,

      // Phase 1: Firmographics (varies by field)
      industry:
        datatype.number(100) > 40 ? random.arrayElement(industries) : undefined,
      revenue_range:
        datatype.number(100) > 50
          ? random.arrayElement(revenueRanges)
          : undefined,
      employee_count:
        datatype.number(100) > 50
          ? datatype.number({ min: 1, max: 10000 })
          : undefined,
      founded_year:
        datatype.number(100) > 60
          ? datatype.number({ min: 1990, max: 2023 })
          : undefined,

      // Phase 1: Social & Enrichment (30% have social profiles)
      social_profiles:
        datatype.number(100) > 70
          ? {
              linkedin: `https://www.linkedin.com/company/${name.toLowerCase().replace(regex, "_")}`,
              ...(datatype.number(100) > 90 && {
                x: `https://x.com/${name.toLowerCase().replace(regex, "")}`,
              }),
              ...(datatype.number(100) > 95 && {
                facebook: `https://facebook.com/${name.toLowerCase().replace(regex, "")}`,
              }),
            }
          : {},
      logo_url:
        datatype.number(100) > 60
          ? `https://logo.clearbit.com/${internet.domainName()}`
          : undefined,

      // Phase 2: External Heartbeat (50% computed)
      external_heartbeat_status:
        datatype.number(100) > 50
          ? random.arrayElement(externalHeartbeats)
          : undefined,
      external_heartbeat_checked_at:
        datatype.number(100) > 50
          ? randomDate(createdAt).toISOString()
          : undefined,

      // Phase 2: Internal Heartbeat (60% computed)
      internal_heartbeat_status: hasHeartbeat
        ? random.arrayElement(internalHeartbeats)
        : undefined,
      internal_heartbeat_score: hasHeartbeat
        ? datatype.number({ min: 0, max: 100 })
        : undefined,
      internal_heartbeat_updated_at: hasHeartbeat
        ? randomDate(createdAt).toISOString()
        : undefined,
