  ...
  try {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0 && !hasError) {
        ...
        sendLog('');
        sendLog('📚 For more help, visit: https://github.com/therealtimex/realtimex-crm/issues');
      }
    ...
    child.on('error', (error) => {
      ...
      sendLog('');
      sendLog('Please try running the migration manually using the CLI instructions.');
      res.end();
    });
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 RealTimeX CRM v${packageJson.version} server running on http://localhost:${PORT}`);
